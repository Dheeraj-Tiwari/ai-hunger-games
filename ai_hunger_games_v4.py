...
class MultiModelArena:
    ...
    async def blind_judge_answers(self, question: str, results: List[RoundResult], 
        ...
        is_open_ended = correct_answer is None or correct_answer.strip() == ""
        
        if is_open_ended:
            judge_instruction = """For this OPEN-ENDED question, be LENIENT. Accept any thoughtful, well-reasoned response.
Mark as "correct": true if the answer is reasonable, coherent, and demonstrates understanding.
Only mark as "correct": false if the answer is nonsensical, off-topic, or completely wrong."""
        else:
            judge_instruction = f"""The correct answer is: {correct_answer}
Mark as "correct": true only if the answer matches or conveys the same meaning as the correct answer."""
...
1. ACCURACY: Is the answer factually correct and reasonable?
...
IMPORTANT: For open-ended questions, be generous. Most thoughtful answers should be marked "correct": true.
Be fair and unbiased. Judge only the content, not the style or source."""
        ...
        try:
            ...
            print(f"   Using fallback judging for open-ended questions...")
            # Fallback: For open-ended questions, accept all non-empty answers
            for result in results:
                if result.answer and len(result.answer.strip()) > 20:
                    # Any substantial answer is considered correct for open-ended questions
                    result.is_correct = True if not correct_answer else (correct_answer.lower() in result.answer.lower())
                    result.accuracy_rating = 7.0  # Default reasonable score
                    result.insight_rating = 7.0   # Default reasonable score
                else:
                    result.is_correct = False
                    result.accuracy_rating = 3.0
                    result.insight_rating = 3.0
            return results
...
"""
    ...
    async def run_round(self, question: str, correct_answer: Optional[str] = None):
        ...
        if correct_answer:
            print(f"📋 Question Type: FACTUAL (correct answer provided)")
        else:
            print(f"📋 Question Type: OPEN-ENDED (judging will be lenient)")
        ...
        # Show all answers received (for transparency)
        print("📝 ALL RESPONSES RECEIVED:\n")
        valid_answers = [r for r in results if not r.error and r.answer]
